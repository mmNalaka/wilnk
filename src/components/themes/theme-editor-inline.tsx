...
const DEFAULT_VARS: Record<string, string> = {
  ...
  "--foreground": toHex("oklch(0.1450 0 0)"),
  "--card": toHex("oklch(1 0 0)"),
  "--card-foreground": toHex("oklch(0.1450 0 0)"),
  "--popover": toHex("oklch(1 0 0)"),
  "--popover-foreground": toHex("oklch(0.1450 0 0)"),
  "--primary": toHex("oklch(0.2050 0 0)"),
  "--primary-foreground": toHex("oklch(0.9850 0 0)"),
  "--secondary": toHex("oklch(0.9700 0 0)"),
  "--secondary-foreground": toHex("oklch(0.2050 0 0)"),
  "--muted": toHex("oklch(0.9700 0 0)"),
  "--muted-foreground": toHex("oklch(0.5560 0 0)"),
  "--accent": toHex("oklch(0.9700 0 0)"),
  "--accent-foreground": toHex("oklch(0.2050 0 0)"),
  "--destructive": toHex("oklch(0.5770 0.2450 27.3250)"),
  "--destructive-foreground": toHex("oklch(1 0 0)"),
  "--border": toHex("oklch(0.9220 0 0)"),
  "--input": toHex("oklch(0.9220 0 0)"),
...
export function ThemeEditorInline({
  ...
  // Prefill non-color fields (typography, radius, shadows) with defaults so they aren't empty
  const buildInitialConfig = useCallback((base?: Record<string, string>) => {
    const nonColorKeys = THEME_GROUPS.flatMap((g) => g.fields)
  ...
  useEffect(() => {
    setName(initialName ?? "My Theme");
    setDescription(initialDescription ?? "");
    setConfig(buildInitialConfig(initialConfig));
  }, [initialName, initialDescription, initialConfig, buildInitialConfig]);
