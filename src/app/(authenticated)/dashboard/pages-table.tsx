  ...
  if (!pages?.length) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">Create your first Link-in-Bio page to get started</p>
      </div>
    );
  }
          ...
          {pages.map((page) => (
            <tr key={page.id} className="border-b">
              <td className="p-4 align-middle">
                <Link href={`/editor/${page.id}`} className="font-medium hover:underline">
                  {page.title || "Untitled"}
                </Link>
              </td>
              ...
              <td className="p-4 align-middle">
                {page.isPublished ? (
                  <Badge className="bg-green-100 text-green-800 border-green-200" variant="outline">Published</Badge>
                ) : page.isPublic ? (
                  <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200" variant="outline">Public</Badge>
                ) : (
                  <Badge className="bg-gray-100 text-gray-800 border-gray-200" variant="outline">Private</Badge>
                )}
              </td>
              <td className="p-4 align-middle text-right tabular-nums">{page.viewCount.toLocaleString()}</td>
              <td className="p-4 align-middle text-right tabular-nums">{page.clickCount.toLocaleString()}</td>
                ...
                <div className="flex justify-end gap-2">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Link href={`/preview/${page.id}`} target="_blank" aria-label="Preview">
